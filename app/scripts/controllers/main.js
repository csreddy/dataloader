...
angular.module('dataloaderApp')
	.controller('MainCtrl', function($scope, $http) {
		...
		function insertDoc(doc, uri, user) {
			...
			return $http({
				method: 'PUT',
				url: 'http://admin:admin@' + location.hostname + ':' + location.port + '/v1/documents?uri=' + uri + '.json' + '&collection=bugs&collection=' + user,
				data: doc,
		...
		function insertUser(doc, uri) {
			...
			return $http({
				method: 'PUT',
				url: 'http://admin:admin@' + location.hostname + ':' + location.port + uri,
				data: doc,
				withCredentials: true
			}).then(function(response) {
				$scope.result.push(uri);
		...
		$scope.loadConfig = function(configData) {
			...
			$http.get('data/config.json').then(function(response) {
				$http({
					method: 'PUT',
					url: 'http://admin:admin@' + location.hostname + ':' + location.port + '/v1/documents?uri=config.json',
					data: response.data,
					withCredentials: true
				}).then(function(response) {
